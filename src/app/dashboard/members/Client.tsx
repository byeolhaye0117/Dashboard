...
export default function Client(p: Props) {
  ...
  */
   ...
   *
   * ── 왜 7일인가 ─────────────────────────────────────────────
   * 그 달에 끝나는 분을 다 임박이라 불렀더니, 9월 1일에 9월 30일 끝나는 분까지
   * 임박으로 떴다. 한 달 내내 딱지가 붙어 있으면 아무도 안 본다.
   * 7일이면 「이번 주에 전화드려야 하는 분」이다 — SOON 한 줄이 그 값이다.
   *
   * ── 어디서부터 7일인가 ─────────────────────────────────────
   * 이번 달을 보고 있으면 오늘부터다. 다른 달로 넘기면 그 달에는 「오늘」이
   * 없으므로 그 달 1일부터 센다 — 달을 넘겨 보는 뜻이 사라지지 않게.
   * 끝은 그 달을 넘지 않는다. 9월을 보고 있는데 10월 3일 끝나는 분이 끼면
   * 「9월 마감 임박」이라는 이름과 어긋난다.
   */
  const 임박창 = useMemo(() => {
    const 달첫날 = `${month}-01`;
    const 시작 = 이달인가 ? now : 달첫날;
    const 끝 = addDays(시작, SOON - 1);
    return { 시작, 끝: 끝 < 달끝 ? 끝 : 달끝 };
  }, [month, 이달인가, now, 달끝]);

  const 임박목록 = useMemo(
    () =>
      끝나는달.filter((m) => {
        const d = (endOf[m.id] ?? "").slice(0, 10);
        return d >= 임박창.시작 && d <= 임박창.끝;
      }),
    [끝나는달, endOf, 임박창]
  );

  /* 세는 규칙 그대로 적는다 — 며칠을 보고 있는지 알 수 없으면, 그 1분이
     내일 끝나는 분인지 이달 말에 끝나는 분인지 모른다 */
  const 임박구간 = useMemo(() => {
    const 짧게 = (d: string) => `${Number(d.slice(5, 7))}/${Number(d.slice(8, 10))}`;
    if (달끝 < now) return "이 달은 이미 다 지났습니다";
    return `${SOON}일 기준 · ${짧게(임박창.시작)} ~ ${짧게(임박창.끝)} 만료`;
  }, [임박창, 달끝, now]);
      ...
      */}
      {peek && (() => {
        ...
        const 이름 =
          ...
          : peek === "마감임박" ? `${SOON}일 안에 끝나는 분 · ${임박구간.replace(`${SOON}일 기준 · `, "")}`
...
function buyPayload(
  ...
  return {
    ...
    결제담당사번: b.담당직원사번,
    /* 받기로 한 전부를 보낸다. 미수금을 빼서 「실제로 받은 돈」만 시트에
       적는 일은 서버가 한 곳에서 한다 — 양쪽에서 빼면 두 번 빠진다 */
    결제금액: String(총액),
    /*
