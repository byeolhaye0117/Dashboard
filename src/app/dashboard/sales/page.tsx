...
async function body() {
  ...
  try {
     ...
     *
     * ── 왜 고쳤나 ──────────────────────────────────────────
     * 여기서 탈퇴를 걸러내니, 이 달에 등록하고 이 달에 그만두신 분이
     * 「등록한 적 없는 사람」이 됐다. 등록성공률의 분자에서 빠지고, 달이
     * 지나 다시 열어 보면 그때와 다른 숫자가 나왔다 — 지난 일이 뒤에서
     * 바뀌는 셈이다. 등록은 일어난 일이니 그대로 센다.
     *
     * 대신 회원상태를 같이 보낸다. 「지금 다니시는 분」을 세는 자리에서는
     * 화면 쪽에서 이 값으로 가른다.
     */
    people = items
      .filter((m) => allowed.has(m.지점코드))
  ...
  try {
    ...
    leads = items
      ...
      .map((c) => ({
        ...
        상담자사번: c["상담자사번"] ?? "",
        /*
         * 명단을 펼치고 한 줄을 눌렀을 때 답해야 하는 값들
         *
         * 「이 사람 왜 등록 안 했지」를 여기서 끝내려면 연락처와 사유·메모까지
         * 있어야 한다. 그것 없이 이름만 보여주면 결국 상담 탭으로 건너가게 된다.
         */
        상담번호: c["상담번호"] ?? "",
        이름: c["이름"] ?? "",
        전화번호: c["전화번호"] ?? "",
  ...
  return (
    <Shell session={session} menus={menus} branches={myBranches} active="매출" crumb="매출"
      ...
      <Client
        ...
        canEditLead={Boolean(ab.get("상담")?.update)}
        problem={problem}
      />
    </Shell>
  );
