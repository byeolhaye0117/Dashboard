/**
 * 답글 지시문 · 견본 · 말투 — naver_place/reply-core.js 에서 옮겨온 사본
 *
 * 원본은 naver_place 저장소에 있고, 그쪽 /api/reply 가 답글의 주인이다.
 * 여기 사본은 그 서버에 닿지 못할 때만 쓴다 — 그 서버가 자고 있거나
 * 아직 새 코드를 못 받았을 때 답글을 아예 못 만들면 안 되기 때문이다.
 *
 * 사본이라 어긋날 수 있다. 그래서 이 길로 만든 답글에는 그 사실을 적어
 * 화면에 알린다. 견본이나 지시문을 고치실 일이 생기면 원본을 고치시고
 * 저에게 말씀해주세요 — 여기도 같이 맞추겠습니다.
 */

/* ── 네이버 입력칸이 받아주는 글자 ────────────────────────────
   답글 칸은 소개글 칸과 달리 이모지를 받는다. m.place 에 실제로 달려 있는
   답글에 😊 가 들어 있는 것을 확인했다. 줄 나눔도 살린다. */
export const NAVER_ALLOWED =
  /[가-힣ㄱ-ㅎㅏ-ㅣa-zA-Z一-鿿ぁ-ゟ゠-ヿ0-9 \n\r\t\-_()&!\[\],.%+~@*^'/?²℃※<>:]/u;

export const EMOJI_OK = /[\u{1F300}-\u{1FAFF}\u{2600}-\u{27BF}\u{2B00}-\u{2BFF}\u{FE0F}\u{200D}]/u;

/* 우리가 만드는 글에서는 아예 안전한 글자로 바꿔서 내보낸다.
   마크다운은 지시문에서 쓰지 말라고 못박아도 AI 가 습관처럼 쓴다.
   네이버 입력칸은 마크다운을 못 읽어서 별표가 글자 그대로 박힌다. */
export const SAFE_SWAP: any[] = [
  [/[·・]/g, ', '], [/[–—]/g, '-'], [/…/g, '...'],
  [/[“”]/g, "'"], [/[‘’]/g, "'"], [/[→←↑↓]/g, '-'],
  [/[「」『』]/g, ''], [/["]/g, "'"],
  [/\*\*([^*\n]+)\*\*/g, '$1'], [/__([^_\n]+)__/g, '$1'],
...
export function replySafe(t: string) {
  let out = (t || '').normalize('NFC');
  SAFE_SWAP.forEach(([re, to]: any) => { out = out.replace(re, to); });
...
export const RV_STOP = /^(있|없|하는|해서|그리|그런|정말|너무|진짜|아주|조금|매우|여기|저기|이곳|그곳|좋습|좋아|감사|고맙|합니|했습|해요|이라|위해|같습|것 ?같)/;

export function reviewStems(review) {
...
export function hasReviewWord(text, review) {
...
export function toneBlock(what){
...
export const LIVELY = `[문장은 이렇게 살려 주세요 - 40대가 쓴 글 말고 20대가 쓴 글처럼]
...
/** 별 셋 이하면 사과 중심이 기본이다 */
export function suggestTone(star) {
...
export function pickFacts(facts, review) {
 ...
 */
export function pickOneReply(text) {
 ...
 */
export function auditReply(text, review, o, star = 5) {
  const t = String(text);
  const info = o || {};
