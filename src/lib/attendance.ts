...
export async function patchAttendance(
   ...
   *
   * ── 무엇이 문제였나 ────────────────────────────────────────
   * 없는 회차를 고르고 아무것도 안 적은 채 저장하면 빈 줄이 하나 생겼다.
   * 그 빈 줄 때문에 달력에 「2」가 붙고, 눌러 열어 보면 2회차에는 아무것도
   * 없었다 — 화면과 창이 서로 다른 말을 했다.
   *
   * 적힌 것이 하나도 없으면 아무 일도 안 한 것이다. 조용히 돌아간다.
   */
  const 적은게있나 = ["출근시각", "퇴근시각", "휴게분", "근무구분", "메모"].some(
    (k) => (changes[k] ?? "").trim()
  );
  if (!적은게있나) return;
