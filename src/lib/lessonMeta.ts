 ...
 *
 * ── 1:1 과 그룹을 무엇으로 가르는가 ─────────────────────────
 * 상품 분류로 가른다. 「수강권」은 1:1, 「그룹수강권」은 그룹수업이다.
 * 파는 자리에서 한 번 정하고, 그 뒤로는 아무도 헷갈리지 않는다.
 *
 * 이름으로 한 번 더 보는 것은 옛 자료를 위해서다. 「그룹수강권」이 생기기
 * 전에 만든 그룹 상품은 아직 분류가 「수강권」이다. 상품 이름에 「그룹」이
 * 들어 있으면 1:1 목록에서는 빼 준다 — 상품 분류를 고치시면 이름과
 * 상관없이 정확해진다.
 *
...
export function fitsKind(productKind: string, name: string, kind: string): boolean {
  ...
  if (kind === KIND_PT) {
    if (k === "그룹수강권") return false;
    if (["1:1PT", "PT", "개인레슨", "퍼스널"].includes(k)) return true;
    return k === "수강권" && !그룹이름;
  }
  if (kind === KIND_GROUP) {
    if (k === "그룹수강권") return true;
