...
function explain(status: number, body: string): Error {
  ...
  /*
   * 프로그램용 계정에는 구글이 저장 용량을 주지 않는다
   *
   * 폴더를 편집자로 공유해도, 파일을 만드는 주체가 이 계정이라 "용량 없음"으로 막힌다.
   * 권한 문제가 아니라 구조 문제여서 공유 설정을 아무리 고쳐도 풀리지 않는다.
   * 이걸 "편집자로 바꿔주세요"라고 안내하면 될 때까지 같은 일을 반복하게 된다.
   */
  if (/storageQuotaExceeded|do not have storage quota/i.test(body)) {
    return new Error(
      "대시보드 계정에는 구글 드라이브 저장 용량이 없어서 사진을 올릴 수 없습니다. " +
        "공유 설정 문제가 아니라 구글의 제한입니다. 다른 방법으로 바꿔야 합니다 — " +
        "이 문장을 그대로 알려주세요."
    );
  }
  if (status === 403) {
