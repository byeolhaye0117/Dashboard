 ...
 * 상품을 네 갈래로 나눈다
 *
 * 이용권: 회원권 · 수강권 · 그룹수강권 · 케어권. 이게 끊기면 회원이 아니다
 * 부가  : 운동복 · 사물함 · 프로틴 같은 것. 돈은 냈지만 이게 살아 있다고
 *         회원권이 살아 있는 것은 아니다
 * 옵션  : 24시 · 여성전용처럼 회원권에 얹는 추가 요금
 * 서비스: 돈을 안 받고 얹어준 것
 *
 * ── 왜 여기로 옮겼나 ───────────────────────────────────────
 * 회원 화면에만 있던 것을 매출 화면에서도 쓰게 됐다. 베껴 두면 한쪽만
 * 고쳐져 두 화면이 다른 말을 한다 — 「재등록」을 회원 화면은 사물함을 빼고
 * 세는데 매출 화면은 넣어 세는 식이다. 이 파일은 구글 접속 코드를 안 물고
 * 있어서 화면(브라우저) 쪽에서도 그대로 쓸 수 있다.
 */
export type Grp = "이용권" | "부가" | "옵션" | "서비스";

export function groupOf(pr?: {
  kind?: string; isService?: boolean; isOption?: boolean;
}): Grp {
  if (!pr) return "이용권";
