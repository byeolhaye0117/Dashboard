...
export type NewConsultation = {
   ...
   * 이 문의를 받은 사람
   *
   * 데스크가 대신 넣어 주는 일이 흔해, 화면에 로그인한 사람과 실제로 전화를
   * 받은 사람이 다를 수 있다. 그래서 고를 수 있게 둔다 — 안 고르면 넣은 사람이다.
   */
  접수자사번?: string;
  /**
   * 실제로 상담해서 등록시킨 사람
   *
   * 접수할 때는 비워 둔다. 아직 상담을 한 것이 아니기 때문이다.
   * 「등록」으로 넘길 때 그 자리에서 고른 사람이 여기 적힌다.
   */
  상담자사번?: string;
  약속일시?: string;
  다음연락예정일?: string;
...
export async function createConsultation(
  ...
  const row: Row = {
    ...
    문의채널: input.문의채널 ?? "",
    방문경로: input.문의채널 ?? "",
    /* 접수자와 상담자는 다른 사람이다 — 문의를 받은 사람과 상담해서 등록시킨
       사람. 접수할 때 상담자를 미리 적어 두면, 등록도 안 된 건에 상담한 사람이
       적혀 있게 된다. 그 자리에서 바로 등록으로 넣는 경우에만 같이 적는다 */
    상담자사번: input.상담자사번 ?? "",
    접수자사번: input.접수자사번 || staffId,
    약속일시: input.약속일시 ?? "",
